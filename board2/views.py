...
def post_new(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            # post.postcode = request.POST.get("postcode", False)
            # post.roadAddress = request.POST.get("road", False)
            # post.jibunAddress = request.POST.get("jibun", False)
            # post.detailAddress = request.POST.get("detail", False)
            post.save()
            return redirect("board2:post_detail", post.pk)
    else:
        form = PostForm()

    return render(
        request,
        "board2/post_new.html",
        {
            "form": form,
            "categories": categories,
            "total_category_count": total_category_post_count,
